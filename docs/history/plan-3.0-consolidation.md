...
> [`docs/history/architecture-review-2026-09.md`](architecture-review-2026-09.md) (every number below is
...
| **M10** | **Release 3.0** ✅ 2026-09-21 (2 commits, `242fa4e` + the release; tag `v3.0.0`) — the full offline gate, the release set, integration, the MCP smoke, the measurement scripts' numbers in `CHANGELOG.md`; the upgrade path to the line documented (`docs/hosts.md`: Foundry ≥ 14.367 first because dnd5e 6.x declares that minimum, then dnd5e 6.x with its migration run out, then nothing on this side; the README requirement and the sandbox version rule point at it); `package.json` 2.2.0 → 3.0.0; design.md §1 / §4 closed; `CONTRIBUTING.md` + `CLAUDE.md` no longer name an open tracker; this tracker archived to `docs/history/` | — | done — **every M0–M9 target restated and met, 2026-09-21 on the sandbox (14.368 / 6.0.3):** results, 24 reads **73,492** chars ≈ 20.4k tokens (target ≤ ~112,600; M2 101,203; M0 154,534), **0** capped, **0** errors; names ×2 **6,153** (≤ 7,000; was 11,147); skill descriptions **7,986** (≤ 8,000; was 15,382); tools/list **201,562** (≤ 230,000; was 283,948), 0 prose offenders, always-on 1,578 (≤ 2,100); `search-compendium` 20 s after connect **353 ms** (< 1 s; was 7.8–13.2 s); `git grep MOLTEN_ src` → `src/hosts/env.ts` + its test; seam 156 handlers, 0 `any` / 0 `unknown`; skills-matrix 0 / 0; Phase-2 and owner-string greps 0 outside `docs/history/` and `.env.example`. **The gate:** offline green (biome · tsc · vitest 98 files / 1,799 tests / 3 skipped · build 530.1 kB · knip); the release set **12 / 12, 490 / 490** (effects-6 51 · region-effects 37 · activities-6 37 · settings-calendar 52 · item-tooling 24 · actor-tooling 48 · pc-build 70 · teleporter-scene-fields 13 · placeables-tooling 87 · scene-tools 24 · cast-activity 25 · region-tooling 22), every script clean in `finally`; integration **7 suites ran, 80 passed / 6 skipped** (154 s); the MCP smoke on the sandbox registration — `get-world-info` (host `local`, role ASSISTANT, 4 books present / 1 missing), a Fighter 3 / Rogue 1 in one `create-pc` call (`success`, HP 35, 40 advancements, 0 unresolved `@scale`), a bad subclass uuid → `success:false` + `errors[1]`, 0 actors persisted (`manage-actors list` → 0) |
...
renaming milestone) → M9 → M10. M0–M7 done 2026-09-20; M8, M9 and M10 done 2026-09-21 — **3.0.0
shipped; the line is closed.**
...
- 2026-09-21 — **M8 the actor projections → `manage-actors` `get-entity` / `export`** — M8 done.
  2 tools → 0 new names, the union 13,381 → **14,539** (+1,158) against the two's **1,356** (−1.3%):
  both take the shared `actorIdentifier` (`get-actor-entity` had `characterIdentifier`,
  `export-actor` `identifier` — the family's one key now; `search-actor-contents`, the read that
  stays its own, keeps `characterIdentifier`); export's two refusals (a relative path, an existing
  file without `overwrite`) are `isError` (they had been "Refused: …" strings in a success shape).
  tools/list **201,562**, names **81** (6,153 chars). `skills-matrix` now treats a union's
  discriminator values (the `action` / `kind` / `type` enums) as names a skill may write in
  backticks beside the tool — `manage-actors` `get-entity` had read as a stale `verb-noun`; the
  `mcp__…__` check stays tool-only. Re-pointed: pc-builder / physical-item-builder /
  session-audit / session-scribe / stat-block-builder / token-cutout (2 lines each), the
  artificer's illustration-builder (1, sibling, uncommitted), manage-activity's and
  update-actor-item's leaves, README (counts + the M8 paragraph closes: "151 tools became 81"),
  verify-actor-export (dispatches the union; the refusal expected as an error),
  verify-actor-read-fixes, the registry tests (83 → 81), the measure test's alias table.
  Live: verify-actor-tooling **48/48** (get-entity added to the dispatch section),
  verify-actor-export 9/9, verify-actor-read-fixes 18/18, verify-read-tools 10/10, integration
  80 passed / 6 skipped. **M8 closes:** names **151 → 81** (target ≤ 90); two-registration name
  chars **11,147 → 6,153** (target ≤ 7,000); tools/list **205,248 → 201,562** (≤ 230,000, every
  family ≤ its own before); the 16 `list/create/update/delete-*` names left are the
...
- 2026-09-21 — **M10 done — 3.0.0 shipped, the line closed.** The gate in `docs/RELEASE.md`
  order on the sandbox (14.368 / 6.0.3, one driver at a time): offline green (biome · tsc ·
  vitest 98 files / 1,799 passed / 3 skipped · build 530.1 kB · knip); the release set 12 / 12
  scripts, **490 / 490** (effects-6 51 · region-effects 37 · activities-6 37 · settings-calendar
  52 · item-tooling 24 · actor-tooling 48 · pc-build 70 · teleporter-scene-fields 13 ·
  placeables-tooling 87 · scene-tools 24 · cast-activity 25 · region-tooling 22), each cleaning
  its `ZZ-*` documents and `worldTime` restored; integration **7 suites ran, 80 passed / 6
  skipped** (154 s; the six are the conditional `ctx.skip()`s of every milestone since M4); the
  MCP smoke on the sandbox registration without a restart (no schema changed since M8, the
  session loaded them fresh) — `get-world-info` host / role / library, the dry run listing 12
  choices and creating nothing, a bad subclass uuid → `success:false` + `errors[1]` with
  `manage-actors list` → 0, then Fighter 3 / Rogue 1 in one call (HP 35, 40 advancements,
  `unresolvedScale: []`), the actor deleted and its folder swept. The numbers, restated against
  the Why table: results 24 reads **73,492** (≤ ~112,600), 0 capped, 0 errors; names ×2
  **6,153** (≤ 7,000); descriptions **7,986** (≤ 8,000); tools/list **201,562** (≤ 230,000;
  leaf 76,976 · descriptions 24,993 · structural 81,951; longest leaf 120; 0 offenders);
  `search-compendium` 20 s after connect **353 ms** (< 1 s; the facet searches ~975 ms; a search
  that is the very first call still waits for the warm-up, 13.8 s today); `MOLTEN_` → env.ts +
  test; seam 156 / 0 / 0; skills-matrix 0 / 0. Landed: `242fa4e` (the upgrade path —
  `docs/hosts.md` "Bringing a world up to the 3.0 line", the README requirement and the sandbox
  version rule pointing at it) and the release commit (`package.json` 3.0.0; `CHANGELOG.md`
  3.0.0 dated with the gate numbers and the upgrade bullet; design.md §1 "was the last feature
  line", §3 decision-#10 note, §4 row ✅ with the headline numbers and the legend without an
  active line; `CONTRIBUTING.md` opening on maintenance, its pins step naming design.md §8 and
  the CHANGELOG instead of a tracker; `CLAUDE.md` still three lines; the three source comments
  re-pointed; this tracker → `docs/history/plan-3.0-consolidation.md` with the history index row
  and its two relative links fixed); tag `v3.0.0` on it. After this there is no tracker: a
  change is a compatibility event, a bug fix, or a premium book (design.md §1, §9).
