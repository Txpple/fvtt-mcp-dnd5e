...
- `scripts/` — the measurement scripts the streams wrote. They import from `dist/` (run
  `npm run build` first) and were written to run from the repo root with the outputs under
  `scratch/`; the ones prefixed `profile-` / `pin-` drive the sandbox (`FOUNDRY_HOST=local`, one
  driver at a time). 3.0's M0 promotes the load-bearing ones to `scripts/measure/`.
