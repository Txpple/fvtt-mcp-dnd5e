...
- **`FOUNDRY_TOOLSETS`** — a registration advertises a subset of the 132 tools (14 named toolsets;
...
commit. First in: `manage-placeables` (`kind`: tiles / lights / walls / drawings / sounds × `action`:
create / list / update / delete) replaces twenty tools for fewer advertised bytes than they cost.
...
The full surface is 132 tools, and `tools/list` for it is ~57k tokens. A client that loads MCP
...
**132 tools total: 122 over the headless bridge (Plane A) + 10 asset file tools over the host's file plane (Plane B).**
