...
- **`FOUNDRY_TOOLSETS`** — a registration advertises a subset of the 117 tools (14 named toolsets;
...
for fewer advertised bytes than they cost.
...
The full surface is 117 tools, and `tools/list` for it is ~57k tokens. A client that loads MCP
...
**117 tools total: 107 over the headless bridge (Plane A) + 10 asset file tools over the host's file plane (Plane B).**
